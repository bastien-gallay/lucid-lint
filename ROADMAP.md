...
| <a id="f-roadmap-toml-source"></a>[F-roadmap-toml-source](#f-roadmap-toml-source) | Architecture | ☐ | v0.2.x | ROADMAP source-of-truth migrates to `.personal/roadmap.toml` (local-first, gitignored); generator emits public `ROADMAP.md` as a build artifact, committed to `main` via single PR on each `v*` tag push. Eliminates worktree+PR ceremony for 10–30 ROADMAP-only edits/week. `/feature-torture` and daily-ops adapters rewritten to read TOML. |
