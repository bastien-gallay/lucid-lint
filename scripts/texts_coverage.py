...
CONDITION = Axis(
    name="Condition",
    keys=[
        "general", "dyslexia", "dyscalculia", "aphasia",
        "adhd", "non-native", "a11y-markup",
    ],
    labels=[
        "`general`", "`dyslexia`", "`dyscalculia`", "`aphasia`",
        "`adhd`", "`non-native`", "`a11y-markup`",
...
TYPE = Axis(
    name="Type",
    keys=[
        "gov_guide", "style_guide", "standard", "research_paper",
        "corpus", "dataset", "tool_fixtures", "sample_collection",
        "textbook", "blog",
    ],
    labels=[
        "`gov_guide`", "`style_guide`", "`standard`", "`research_paper`",
...
def count_matrix(
    sources: list[dict], row: Axis, col: Axis
        ...
        for r in _as_list(row.of(src)):
            ...
            for c in _as_list(col.of(src)):
                ...
                if is_public:
                    public[(r, c)] += 1
    return {
        (r, c): Cell(public=public[(r, c)], total=total[(r, c)])
        for r in row.keys
        for c in col.keys
    }
...
def render_full_doc(sources: list[dict]) -> str:
    ...
    for cond in CONDITION.keys:
        for lang in LANG.keys:
            ...
            if not entries:
                continue
            items = ", ".join(
                f"{s['title']} (`{s['redistribution']}`)" for s in entries
            )
            gap_lines.append(f"| `{cond}` | {LANG_LABELS[lang]} | {items} |")
    gap = "\n".join(gap_lines) if len(gap_lines) > 2 else (
    ...
    return "\n".join([
        ...
        "  replacements for.",
        "",
        "`examples/local/` is gitignored; this document stays off GitHub"
        " on purpose.",
        "",
        "## Shape × language",
        "",
...
def splice(old: str, section: str) -> str:
    ...
    if BEGIN_MARKER not in old or END_MARKER not in old:
        raise RuntimeError(
            f"Expected both {BEGIN_MARKER} and {END_MARKER} in examples/texts.md"
        )
...
def main() -> int:
    ...
    public_updated = splice(
        TEXTS_MD.read_text(encoding="utf-8"), public_section
    )
