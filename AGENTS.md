...
- **GitHub Actions are pinned to full-length commit SHAs** — never `@vX.Y.Z` in `.github/workflows/`. Append a version comment for readability: `actions/checkout@de0fac2e4500dabe0009e67214ff5f5447ce83dd # v6.0.2`. Surfaced by [`.github/workflows/scorecard.yml`](.github/workflows/scorecard.yml) (OpenSSF Scorecard's `Pinned-Dependencies` check downgrades the score and emits SARIF when an unpinned ref reappears). Treat any code-review suggestion that rewrites a SHA-pinned action back to a tag ref as a regression — keep the SHA, update the trailing version comment if the pin is bumped.
...
- **Always work from an independent git worktree**: never modify files directly on the main checkout. Create a dedicated worktree per branch (`git worktree add ../lucid-lint-<slug> -b feat/<slug>`) so parallel work cannot collide on the index, on `target/`, or on uncommitted edits. One worktree per logical change; remove it (`git worktree remove`) once the branch is merged.
- **Splitting a feature into sub-features**: default to **not splitting** — keep one ROADMAP row with a checklist. Sub-IDs earn their cost only when items ship on independent timelines, get cited from elsewhere, or get prioritised differently in MoSCoW. When a split is warranted, coin sub-slugs as `<parent-slug>-<descriptor>` (e.g. `F-fix-mode-redundant-intensifier` under `F-fix-mode`). The parent stays as the umbrella narrative entry with cross-refs to children; each child gets its own `<a id="f-...">` anchor and the standard surface contract (ROADMAP row + CHANGELOG entry on land). Legacy numeric splits (`F35a` / `F35b` / `F78b` / `F105b`) are grandfathered — letter-suffix form is closed for new entries.
...
- [ ] If any `docs/src/en/**/*.md` page changed, `just docs-lang-staleness` is clean — either the FR counterpart's `en-source-sha` stamp was updated in the same commit, or a follow-up FR stamp commit lands before the PR merges. Stale stamps fail CI on PRs.
