...
- **ROADMAP feature IDs & splitting** → [CONTRIBUTING.md § Roadmap conventions](CONTRIBUTING.md#roadmap-conventions). In short: new entries use `F-<kebab-slug>` (legacy `F<number>` unchanged); slugs are coined locally; uniqueness is enforced by [`tests/roadmap_id_uniqueness.rs`](tests/roadmap_id_uniqueness.rs); default to **not** splitting a feature into sub-IDs.
