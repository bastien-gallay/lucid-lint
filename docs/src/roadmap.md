...
| <a id="f-security-hardening"></a>[F-security-hardening](#f-security-hardening) | **Security & Supply Chain hardening — cargo-deny + gitleaks.** **cargo-deny** replaces `cargo-audit` for faster, unified checks (CVEs + Licenses + Crate Bans); **gitleaks** as a pre-commit hook to prevent secret leaks. | 🔴 Next | Static-analysis session 2026-05-04 |
| <a id="f-infra-audit"></a>[F-infra-audit](#f-infra-audit) | **Infrastructure & API audit — actionlint + cargo-semver-checks.** **actionlint** for workflow validation; **cargo-semver-checks** to protect the `[lib]` API surface before major releases. | 🔴 Next | Static-analysis session 2026-05-04 |
