...
| [F-repo-config-hardening](#f-repo-config-hardening) | Architecture | ☐ | v0.2.x | One-time GH Settings hardening — tag ruleset on `v*`, Dependabot version updates, Actions pinned-SHA + Dependabot `github-actions` ecosystem, secret-scanning push protection, CodeQL default setup |
...
| Architecture / scoring | 5 ([F-project-scoring-rollup](#f-project-scoring-rollup), [F-experimental-rule-status](#f-experimental-rule-status), [F143](#f143), [F-roadmap-slug-ids](#f-roadmap-slug-ids), [F-repo-config-hardening](#f-repo-config-hardening)) | 1 ([F-tight-list-paragraphs](#f-tight-list-paragraphs)) | [F-section-scoring](#f-section-scoring), [F-reading-time-score](#f-reading-time-score) | [F-per-family-subscores](#f-per-family-subscores), [F-section-scoring](#f-section-scoring), [F-score-letter-grade](#f-score-letter-grade), [F-score-traffic-light](#f-score-traffic-light) | [F-reading-time-score](#f-reading-time-score) |
...
| [F53](#f53) | `readability.large-number-unanchored` | Readability | `dyscalculia`, `general` | CDC CCI | Ships as `Experimental` in v0.2.x via [F-experimental-rule-status](#f-experimental-rule-status); flips to `Stable` at v0.3 cut. |
| [F57](#f57) | `syntax.parenthetical-depth` | Syntax | `adhd`, `general` | plainlanguage.gov, Hemingway | Ships as `Experimental` in v0.2.x via [F-experimental-rule-status](#f-experimental-rule-status); flips to `Stable` at v0.3 cut. |
...
| <a id="f-repo-config-hardening"></a>[F-repo-config-hardening](#f-repo-config-hardening) | **GitHub repo config hardening — one-time Settings pass.** Routed 2026-05-03 (repo-config session). Project ships from `main` with tag-triggered crates.io publish (CI workflow keys off `v*` tag push); current `main-protection` ruleset covers branch but tags + supply-chain surfaces are unprotected. Bundled because all items are GH Settings clicks (no code, no PR, all reversible). **Checklist:** (1) **Tag ruleset** on `v*` pattern — block deletion + force-push; protects the release-flow trigger. (2) **Dependabot version updates** — `.github/dependabot.yml` with `cargo` (weekly) + `github-actions` (weekly) ecosystems; the `github-actions` ecosystem complements item (3) by PR'ing pinned-SHA bumps with the human-readable version diff in the body. (3) **Actions: Require pinned-SHA** toggle — Settings → Actions → General → "Require actions to be pinned to a full-length commit SHA"; closes the re-tag attack on third-party actions. (4) **Secret scanning + push protection** — Settings → Code security; relevant given `CARGO_REGISTRY_TOKEN` lives in repo secrets. (5) **CodeQL default setup** — free for public repos, weekly Rust scan, catches `unwrap`-in-lib regressions and similar. (6) **Private vulnerability reporting** — give security researchers a private channel before public issues. **Estimated effort:** ~30 min for the whole pass; items independent — can ship piecewise. **Not in scope:** branch-target expansion of `main-protection` (no release branches exist; revisit on first backport). | 🔴 Next | Repo-config session 2026-05-03 |
