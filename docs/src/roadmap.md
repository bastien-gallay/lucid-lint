...
[F35b](#f35b)/F35c, [F104](#f104), [F105](#f105), [F107](#f107), [F110](#f110)–[F112](#f112), [F123](#f123) all shipped. v0.3 is
...
| v0.2.0 | ✅ Released 2026-04-22 | Yes (rule-id harmonisation) | Hybrid scoring ([F14](#f14)), SARIF ([F32](#f32)), condition tags ([F71](#f71)/F72), 8 new rules (25 total), [F10](#f10) EN/FR auto-formula |
...
| **v0.2.x** | 🚧 **In progress** | No | FR translations ([F25](#f25) ✅ closed 2026-05-01 — per-rule 25/25 + guides 8/8 + architecture 2/2 + contributing), responsive ([F34](#f34)), P2 a11y ([F35b](#f35b) — [F35c](#f35c) closed 2026-05-01 as audit false-positive), [F84](#f84) part 2, perf ([F102](#f102)/F103 — [F93](#f93)/F94 refuted by profile 2026-04-25), hygiene ([F95](#f95)/F96), [F15](#f15) project roll-up, adoption channels ([F110](#f110)/F111/F112) |
| **v0.3** | ☐ Scoped | **Yes** | [F22](#f22) v0.3 slice, [F10](#f10) remainder, 5 condition-tag rules ([F46](#f46)/F49/F51/F53/F57) |
...
| Docs site (bilingual / content / theming / reading) | 3 ([F25](#f25), [F30](#f30), [F34](#f34), [F35b](#f35b)) | — | — | [F36](#f36), [F43](#f43), [F44](#f44), [F73](#f73), [F89](#f89), [F90](#f90)/F91 | — |
...
| Developer experience | — | [F88](#f88) (narrow `--fix`) | LSP server | [F73](#f73)-dx (`--compare`), [F79](#f79) | [F12](#f12) |
...
| [F12](#f12) — score-evolution dashboard | CI users explicitly ask for trend view (not delta — delta is [F73](#f73)-dx / `--compare`) |
...
| <a id="f117"></a>[F117](#f117) | **W3C Cognitive and Learning Disabilities Accessibility Task Force (COGA) tools survey submission.** Submit `lucid-lint` next time COGA refreshes its public tools list. High credibility, slow cadence, low effort once the submission template is identified. | 🟢 Could | Adoption-channels brainstorm 2026-04-25 |
| <a id="f118"></a>[F118](#f118) | **Conference talk submission.** Targets: Write the Docs (Portland or Australia), RustNation 2027, EuroRust 2026, A11y Camp. CFP-driven, variable effort. The "from samply profile to 12% perf win" arc from 2026-04-25 plus the bilingual-cognitive-a11y angle is talk-shaped. | 🟢 Could | Adoption-channels brainstorm 2026-04-25 |
| <a id="f119"></a>[F119](#f119) | **Mastodon / Bluesky cadence.** Modest (1–2 posts/week), tag accessibility + tech-writing communities. Cheap, ongoing; effective when paired with [F113](#f113) audit-PR landings or [F110](#f110) pack release as anchors. | 🟢 Could | Adoption-channels brainstorm 2026-04-25 |
...
until first user asks. "FALC-ready" badge program parked behind [F115](#f115)
(needs Inclusion Europe coordination first; do not squat on their
logo). Hacker News "Show HN" parked until at least one of
[F110](#f110) / [F111](#f111) / [F113](#f113) has landed an anchor. Co-authored academic letter
...
| [F110](#f110) | Adoption channels | Vale style pack (subset of rules → `vale-cli/packages` topic) |
| [F111](#f111) | Adoption channels | DINUM submission to `accessibilité.numerique.gouv.fr` resources |
| [F112](#f112) | Adoption channels | Three awesome-list PRs (`awesome-a11y`, `awesome-writing-tools`, `awesome-rust`) |
...
| [F113](#f113) | Adoption channels | Audit-and-PR play on one famous OSS doc target |
...
**[F116](#f116)** (mdbook-lint coexistence guide), **[F117](#f117)** (W3C COGA tools
survey submission), **[F118](#f118)** (conference talk — RustNation /
EuroRust / Write the Docs CFP), **[F119](#f119)** (Mastodon / Bluesky cadence),
...
| [F113](#f113) | **Mixed-script test fixtures.** Pin behaviour on EN + CJK and LTR + RTL prose mixed within one paragraph. `unicode_words()` should handle the boundaries correctly (UAX-29), but no regression test exists. Filed as Speculative — no known bug, just a coverage gap. Open if a real-world bilingual corpus surfaces edge cases. | 🟢 Speculative | 2026-04-25 encoding survey |
...
|---|---|---|
| ✅ | Minimal inline-disable | `<!-- lucid-lint disable-next-line <rule-id> -->` for Markdown inputs, single rule id, optional reason. See [RULES.md → Suppressing diagnostics](https://github.com/bastien-gallay/lucid-lint/blob/main/RULES.md#suppressing-diagnostics). Block form, config ignores, file-level scope and required `reason=` are tracked as [F18](#f18)–[F21](#f21) below. |
