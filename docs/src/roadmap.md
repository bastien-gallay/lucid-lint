...
| F108 | Quality — tests | Assert reported ratio in `low_lexical_diversity` tests (F98 follow-up, 36 mutants) |
| F109 | Quality — tests | Borderline-cluster fixtures for `low_lexical_diversity::check` (F98 follow-up, 11 mutants) |
...
| F108 | **`low_lexical_diversity::ratio_at_anchor_min` — assert reported ratio in tests** (mutation-testing follow-up from F98 baseline). 36 of the 47 missed mutants on `low_lexical_diversity.rs` cluster in this helper (lines 204–240). The function returns the minimum ratio across the slide; that number ends up *only* in the diagnostic message string — no test asserts on it, so arithmetic mutations (`-= → +=`, `< → ==`, `/ → %`, …) shift the reported figure without breaking emission. Add a test that parses the emitted `message` and asserts on the numeric value at known fixture anchors. Bonus: extract the ratio onto the diagnostic struct as a typed field once F15 (project-level scoring roll-up) lands, so assertions don't need string parsing. | 🔴 Next | F98 baseline 2026-04-25 |
| F109 | **`low_lexical_diversity::check` — borderline-cluster fixtures** (mutation-testing follow-up from F98 baseline). 11 of the 47 missed mutants cluster in the `check` impl (lines 87–122) on threshold edges (`< → <=`, `< → ==`). Current corpus exercises clearly-clustered and clearly-clean prose; nothing sits at the boundary. Add 2–3 short fixtures right at the threshold (window size = N, ratio = exactly `min_ratio`, etc.) so a `<` → `<=` flip changes diagnostic emission. | 🔴 Next | F98 baseline 2026-04-25 |
