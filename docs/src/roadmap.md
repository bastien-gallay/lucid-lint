...
| F25 | Docs — bilingual | FR per-rule pages (13 / 25 shipped — `structure` + `rhythm` categories 100 % FR-complete) |
...
| F123 | Distribution | Curl-pipe-sh installer (cargo-dist `shell` + `powershell` flip) |
...
| ID | Topic | Item |
|---|---|---|
| F25 | Docs — bilingual | Per-rule FR pages + guides (13 / 25 shipped — `structure` + `rhythm` categories 100 % FR-complete) |
| F34 | Docs — reading prefs | Responsive / mobile adaptation |
| F35b | Docs — reading prefs | Drop `role="radiogroup"` on reading chips (P2 a11y) |
| F35c | Docs — reading prefs | Reduced-motion colour-tint preservation (P2 a11y) |
...
| F123 | **Curl-pipe-sh installer (cargo-dist generated)** — promoted to 🔴 Next from 2026-04-27 Block E recon, targeted at the next v0.2.x patch. Quick win: we already ship via `cargo-dist` v0.25.1, so enabling `shell` and `powershell` installers is a config flip in `Cargo.toml` `[workspace.metadata.dist]` — no new infrastructure. Output is a signed `lucid-lint-installer.sh` attached to each GitHub Release that probes `uname`, picks the matching tarball from our five targets (`x86_64-unknown-linux-gnu`, `x86_64-unknown-linux-musl`, `aarch64-apple-darwin`, `x86_64-apple-darwin`, `x86_64-pc-windows-msvc`), verifies SHA256, drops the binary in `~/.cargo/bin` (cargo-dist default), installs shell completions. Public URL: `curl -LsSf https://github.com/<org>/lucid-lint/releases/latest/download/lucid-lint-installer.sh \| sh`. Pin via `LUCID_LINT_VERSION=`. Definition of done: enabled, tested on all five platforms in CI, documented in README + a docs page covering the `--check` / inspect path (curl-pipe-sh remains controversial — surface the audit route prominently). Risk: a vanity `sh.lucid-lint.dev` redirect is a v0.5 concern, not a blocker. | 🔴 Next | 2026-04-27 Block E recon |
