...
- **F57 — `syntax.parenthetical-depth` (experimental,
...
- **F53 — `readability.large-number-unanchored`
...
- **F-prose-audits — pre-commit + CI for `typos` (spell
...
- **F51 — `structure.number-run` (experimental, cohort
...
- **Adversarial-review wins — three landed actions from
...
- **ROADMAP routing — F-report-quick-wins (🔴 Next) and
...
- **F46 — `lexicon.homophone-density` — paragraph-level
...
- **F-repo-config-hardening closed — GitHub repo
...
- **F-report-quick-wins — TTY report quick-wins block.**
...
- **CI MSRV read dynamically from `Cargo.toml`.** The
...
- **`just setup` warns when `uv` is missing.** Added a
